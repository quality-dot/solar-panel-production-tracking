...
describe('ShellTool', () => {
    ...
    describe('execute', () => {
        ...
        it('should return error with error property for invalid parameters', async () => {
            const result = await shellTool.execute({ command: '' }, // Empty command is invalid
            mockAbortSignal);
            expect(result.llmContent).toContain('Could not execute command due to invalid parameters:');
        ...
        it('should return error with error property for invalid directory', async () => {
            vi.mocked(fs.existsSync).mockReturnValue(false);
            const result = await shellTool.execute({ command: 'ls', directory: 'nonexistent' }, mockAbortSignal);
            expect(result.llmContent).toContain('Could not execute command due to invalid parameters:');
